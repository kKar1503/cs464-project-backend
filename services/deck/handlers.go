...
// cardRow is the subset of fields shared by all four GetAllCards* row types
// generated by sqlc. We normalise them here so the handler loop stays compact.
type cardRow struct {
	CardID      int32
	CardName    string
	Affiliation int32
...
func handleGetAllCards(w http.ResponseWriter, r *http.Request) {
	...
	case rarity != "" && affiliation != "":
		...
		res, e := queries.GetAllCardsByRarityAndAffiliation(ctx, db.GetAllCardsByRarityAndAffiliationParams{
			Rarity:      rarity,
			Affiliation: int32(aff),
		})
		err = e
		for _, c := range res {
			rows = append(rows, cardRow{c.CardID, c.CardName, c.Affiliation, c.Rarity, c.ManaCost, c.Description, c.IconUrl})
		}
	case rarity != "":
		res, e := queries.GetAllCardsByRarity(ctx, rarity)
		err = e
		for _, c := range res {
			rows = append(rows, cardRow{c.CardID, c.CardName, c.Affiliation, c.Rarity, c.ManaCost, c.Description, c.IconUrl})
		}
	case affiliation != "":
		var aff int64
		fmt.Sscanf(affiliation, "%d", &aff)
		res, e := queries.GetAllCardsByAffiliation(ctx, int32(aff))
		err = e
		for _, c := range res {
			rows = append(rows, cardRow{c.CardID, c.CardName, c.Affiliation, c.Rarity, c.ManaCost, c.Description, c.IconUrl})
		}
	default:
		res, e := queries.GetAllCards(ctx)
		err = e
		for _, c := range res {
			rows = append(rows, cardRow{c.CardID, c.CardName, c.Affiliation, c.Rarity, c.ManaCost, c.Description, c.IconUrl})
		}
	...
	cards := make([]CardResponse, 0, len(rows))
	for _, c := range rows {
...
type PlayerCardResponse struct {
	...
	// Scaled stats at the current level (what the player actually uses in-game).
	Attack int `json:"attack"`
	HP     int `json:"hp"`
	// Base stats at level 1, included so the frontend can show the delta.
	BaseAttack int `json:"base_attack"`
	BaseHP     int `json:"base_hp"`
	// Cost to reach the next level. Zero when already at max level.
	NextLevelCardsCost    int32 `json:"next_level_cards_cost"`
	NextLevelCrystalsCost int32 `json:"next_level_crystals_cost"`
	// Crystals refunded per copy when disenchanting. Only spendable at max level.
	DisenchantCrystalsPerCopy int32 `json:"disenchant_crystals_per_copy"`
}

// buildPlayerCardResponse enriches a row with scaled stats and next-level cost.
// Used by both /players/me/cards and /players/me/cards/available.
func buildPlayerCardResponse(
	cardID int32, name string, affiliation int32, rarity string,
	manaCost int32, description, iconURL string,
	maxLevel, level, quantity int32, baseAtk, baseHP int32,
) PlayerCardResponse {
	var cardsCost, crystalsCost int32
	if level < maxLevel {
		cardsCost, crystalsCost = LevelUpCost(int(level))
	}
	return PlayerCardResponse{
		...
		Attack:                ScaleStat(int(baseAtk), int(level)),
		HP:                    ScaleStat(int(baseHP), int(level)),
		BaseAttack:                int(baseAtk),
		BaseHP:                    int(baseHP),
		NextLevelCardsCost:        cardsCost,
		NextLevelCrystalsCost:     crystalsCost,
		DisenchantCrystalsPerCopy: DisenchantValue(rarity),
	}
}
...
func handleGetPlayerCards(w http.ResponseWriter, r *http.Request) {
	...
	for _, c := range dbCards {
		cards = append(cards, buildPlayerCardResponse(
			c.CardID, c.CardName, c.Affiliation, c.Rarity, c.ManaCost,
			c.Description, c.IconUrl, c.MaxLevel, c.Level, c.Quantity,
			int32(c.BaseAttack), int32(c.BaseHp),
		))
	}
...
func handleGetCardsNotInDecks(w http.ResponseWriter, r *http.Request) {
	...
	for _, d := range decks {
		...
		for _, c := range dbCards {
			cards = append(cards, buildPlayerCardResponse(
				c.CardID, c.CardName, c.Affiliation, c.Rarity, c.ManaCost,
				c.Description, c.IconUrl, c.MaxLevel, c.Level, int32(c.Quantity),
				int32(c.BaseAttack), int32(c.BaseHp),
			))
		}
...
// validateActiveDeckForMatchmaking is the stricter check used before a player
// is allowed to queue for a game. In addition to the standard deck-composition
// rules (enforced on edit via validateDeckCards), the queue requires the deck
// to be a full fighting deck — exactly maxDeckSize cards — because anything
// smaller can't sustain the draw-pile top-up loop. Leveling up / disenchanting
// can silently prune a deck below that threshold even though it was valid
// when first saved.
//
// Returns nil when the user is queue-ready, or a user-facing error describing
// what's wrong so the client can redirect to the deck builder.
func validateActiveDeckForMatchmaking(ctx context.Context, userID int64) error {
	activeDeckID, err := queries.GetActiveDeck(ctx, userID)
	if err != nil {
		return fmt.Errorf("failed to load active deck")
	}
	if activeDeckID == nil {
		return fmt.Errorf("no active deck set")
	}

	deckCards, err := queries.GetDeckCards(ctx, *activeDeckID)
	if err != nil {
		return fmt.Errorf("failed to load deck cards")
	}
	if len(deckCards) != maxDeckSize {
		return fmt.Errorf("active deck must have exactly %d cards, currently has %d", maxDeckSize, len(deckCards))
	}
	...
	return validateDeckCards(ctx, userID, cardIDs)
}

// Internal endpoint — returns whether the user's active deck is legal for
// matchmaking. Called by the matchmaking service before it lets a player
// join the queue. Always returns 200 with {valid: bool, reason: string}.
func handleValidateActiveDeckForMatchmaking(w http.ResponseWriter, r *http.Request) {
	...
	if err := validateActiveDeckForMatchmaking(r.Context(), userID); err != nil {
